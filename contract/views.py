...
def generate_multi_contract_excel_data(contract_detail):
    try:
        ...
        contract_data = {"Assuré": contract_detail["insuree"]["lastName"] + " " + contract_detail["insuree"][
            ...
            "Numéro CAMU": str(contract_detail["insuree"]["camu_number"]) if contract_detail.get("insuree") and
                                                                             contract_detail["insuree"].get(
                                                                                 "camu_number") else '',
            "N° d'ins. du Resp": contract_detail["insuree"]["chfId"] if contract_detail.get("insuree") and
                                                                        contract_detail["insuree"].get("chfId") else '',
            "Ensemble du plan de contribution": contract_detail["contributionPlanBundle"]["code"] + " - " +
                                                contract_detail["contributionPlanBundle"][
                                                    "name"] if contract_detail.get("contributionPlanBundle") and
                                                               contract_detail["contributionPlanBundle"].get("code") and
                                                               contract_detail["contributionPlanBundle"].get(
                                                                   "name") else '',
            "Gross Salary": str(contract_detail["jsonExt"]["calculation_rule"]["income"]) if contract_detail.get(
                "jsonExt") and contract_detail["jsonExt"].get("calculation_rule") and contract_detail["jsonExt"][
                                                                                                 "calculation_rule"].get(
                "income") else '', "Cotisation de l'employeur": str(
                contract_detail["customField"]["employerContribution"]) if contract_detail.get("customField") and
                                                                           contract_detail["customField"].get(
                                                                               "employerContribution") else '',
            "Cotisation des employés": str(contract_detail["customField"]["salaryShare"]) if contract_detail.get(
...
def get_contract_custom_field_data(detail):
    ...
    phn_json = PolicyHolderInsuree.objects.filter(insuree_id=insuree.id, policy_holder__code=policy_holder.code,
        policy_holder__date_valid_to__isnull=True, policy_holder__is_deleted=False, date_valid_to__isnull=True,
    ...
    contract_data = {'id': detail.id, 'jsonExt': detail.json_ext or '',  # Set empty string if json_ext is None
        'contract': {'id': detail.contract.id if detail.contract else '',  # Set empty string if contract is None
            ...
            'lastName': insuree.last_name if insuree else '', 'otherNames': insuree.other_names if insuree else '', },
        'contributionPlanBundle': {'id': cpb.id if cpb else '',  # Set empty string if cpb is None
            ...
            'periodicity': cpb.periodicity if cpb else '', 'dateValidFrom': cpb.date_valid_from if cpb else '',
            'dateValidTo': cpb.date_valid_to if cpb else '', 'isDeleted': cpb.is_deleted if cpb else '',
            'replacementUuid': cpb.replacement_uuid if cpb else '', }, 'customField': custom_field_data, }
    return contract_data
